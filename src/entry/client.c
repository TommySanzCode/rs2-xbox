...
#ifdef NXDK
static CacheCrcStatus xbox_cache_crc_status = CACHE_CRC_OK;
#endif
...
#ifdef NXDK
    client_draw_progress(c, "Loading local cache checksums", 10);
    xbox_cache_crc_status = cachecrc_load("D:\\cache\\client\\crc", c->archive_checksum);
    if (xbox_cache_crc_status != CACHE_CRC_OK) {
        rs2_error("D:\\cache\\client\\crc: %s\n", cachecrc_status_message(xbox_cache_crc_status));
...
#ifdef NXDK
        extern uint32_t xbox_last_present_ms;
        sprintf(buf, "Display fit: %lu ms", (unsigned long)xbox_last_present_ms);
...
void client_draw_error(Client *c) {
    ...
    if (c->error_loading) {
        ...
        if (xbox_cache_crc_status != CACHE_CRC_OK) {
            ...
            platform_draw_string("Unable to load the Xbox cache", 30, 35);
            platform_set_font("Helvetica", true, 12);
            platform_set_color(WHITE);
            platform_draw_string(cachecrc_status_message(xbox_cache_crc_status), 30, 85);
            platform_draw_string("Required file: D:\\cache\\client\\crc", 30, 115);
            platform_draw_string("Rebuild and copy the complete game folder, then restart.", 30, 155);
            return;
        }
#endif
        int y = 35;
